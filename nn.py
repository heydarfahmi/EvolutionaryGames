...
class NeuralNetwork():
    ...
    def __init__(self, layer_sizes):
        ...
        # self.set_layer(hidden_layer_size, hidden_layer_size)
        self.set_layer(output_layer_size, hidden_layer_size)
        # TODO

    def set_layer(self, layer_size, last_layer_size):
        self.activations.append(np.zeros((layer_size, 1), dtype=np.float64))
        self.bias.append(np.random.randn(layer_size, 1).astype(np.float64))
        self.weights.append(np.random.randn(layer_size, last_layer_size).astype(np.float64))
